...
BuildGui:
  ...
  Gui, 1:Add, Tab, w287 h145 vTab1 gTab1 AltSubmit -Wrap , Simple|Advanced|Mouse|Options|Info
    ...
    Gui, 1:Tab, Advanced
      ...
      Gui, 1:Add, Button, x+28 yp-1 vBtnShowList gBtnShowList -Wrap, % ChkShowList ? "<< Less Info" : "More Info >>"
...
PauseAction:                      ;user has pressed toogle auto-update hotkey
  If ChkHideGui {                 ;user wants hide/show toggle
      ...
      If ChkShowInfoToolTip
          ToolTip(UpdateIsStopped ? ("Press " . HtkPauseAction . " to hide GUI") : ("Wait for " . HtkPauseAction . " key to`nupdate data and show GUI"),2000)
  }Else {
      ...
      If ChkShowInfoToolTip                   ;give feedback
          ToolTip(Scriptname . "`n----------------------------`nUpdate data : " . (UpdateIsStopped ? "Off" : "On"))
    }
...
ShowHideList:
  GuiControl, 1:, BtnShowList, % ChkShowList ? "<< Less Info" : "More Info >>"
      ...
      If ( MouseScreenX MouseScreenY <> OldPosition or MouseColorRGB <> OldMouseColorRGB){    ;only update color picker when mouse moves or color changes
          ...
          Loop, %Dim% {
              ...
              Loop, %Dim% {
                  ...
                  ;ColorRGB := fastPixelGetColor(x,y,RadColorPick = 1)
                  ColorRGB := fastPixelGetColor(x,y)
                  If RadColorPick = 1
                    ColorRGB := ((ColorRGB&0xFF) << 16) | (ColorRGB&0xFF00) | (ColorRGB >> 16)
