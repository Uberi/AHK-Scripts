...
Suspend, On
MsgBox, 36, Start, Would you like to start the mail bot?
IfMsgBox, Yes
{
 MsgBox, 64, Info, Press "Esc" to stop the bot`, and "F1" to pause it.
 Gosub, Bot
}
ExitApp
...
Loop
{
 Run, https://signup.mail.com/,, Max UseErrorLevel
 Sleep, 2000
 ToolTip, Wait until the page finishes loading`, and then press "Enter"
 Suspend, Off
 KeyWait, Enter, D
 Suspend, On
 GoToWindow("Chrome,Mozilla,Internet")
 Sleep, 1000
 Send, {Tab 4}
 ...
 Send, mail.com
 Send, {Tab}
 UserName := FirstName . Generate()
 Send, %UserName%
 Send, {Tab}{Down}{Tab 2}
 Password := RandomName() . Generate()
 Send, %Password%                           
 Send, {Tab}
 Send, %Password%
 Send, {Tab}{Down 2}{Tab}
 SecurityAnswer := RandomName()
 Send, %SecurityAnswer%
 ...
 Loop, %BirthDay%
  Send, {Down}
 Send, {Tab}
 Random, BirthYear, 14, 30
 Loop, %BirthYear%
  Send, {Down}
 Send, {Tab}{PGDN}
 ToolTip, Fill in the "CAPCHA"`, and then press "Enter"
 Suspend, Off
 KeyWait, Enter, D
 Suspend, On
 Sleep, 1000
 ...
 Send, {Enter}
 Sleep, 2000
 ToolTip, Press the button that says "You Have New Mail"`, and then press "Enter"
 Suspend, Off
 KeyWait, Enter, D
 Suspend, On
 Sleep, 2000
 ToolTip, Press the button that says "Start Using Mail.com"`, and then press "Enter"
 Suspend, Off
 KeyWait, Enter, D
 Suspend, On
 ToolTip, Wait until the page finishes loading`, and then press "Enter"
...
Save:
CredLog = UserName: %UserName%@mail.com`nPassword: %Password%`n`nName: %FirstName% %LastName%`nSecurity Answer: %SecurityAnswer%`nCity: %City%`nPostal Code: %PostalCode%`n`n`n
FileAppend, %CredLog%, Log.txt
CredLog = 
Return
...
RandomCity()
{
 CityList = Barrie/Belleville/Brampton/Brant/Brantford/Brockville/Burlington/Cambridge/Chatham-Kent/Clarence-Rockland/Cornwall/Dryden/Elliot Lake/Greater Sudbury/Guelph/Haldimand County/Hamilton/Kawartha Lakes/Kenora/Kingston/Kitchener/London/Mississauga/Niagara Falls/Norfolk County/North Bay/Orillia/Oshawa/Ottawa/Owen Sound/Pembroke/Peterborough/Pickering/Prince Edward County/Port Colborne/Quinte West/Sarnia/Sault Ste. Marie/St. Catharines/St. Thomas/Stratford/Temiskaming Shores/Thorold/Thunder Bay/Timmins/Toronto/Vaughan/Waterloo/Welland/Windsor/Woodstock
 Random, RandLine, 1, 51
 Loop, Parse, CityList, /
 {
  If A_Index = %RandLine%
   Return, A_LoopField
...
RandomPostalCode()
{
 ...
 Loop, 6
 {
  If A_Index In 1,3,5
  {
   Random, RandLetter, 1, 26
   Loop, Parse, Alphabet
   {
    If A_Index = %RandLetter%
    {
     RandLetter = %A_LoopField%
     Break
    }
   }
  }
  Else
   Random, RandLetter, 0, 9
  PostCode = %PostCode%%RandLetter%
 }
 Return, PostCode
}
