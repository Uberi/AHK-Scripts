...
GL_WRAPPER_VERSION := 2.2

if (A_IsUnicode)
{
  hOpenGL := DllCall("LoadLibraryW", "wstr", "opengl32", "uint")
  hGLU := DllCall("LoadLibraryW", "wstr", "glu32", "uint")
  hGDI := DllCall("LoadLibraryW", "wstr", "gdi32", "uint")
}
else
{
  hOpenGL := DllCall("LoadLibraryA", "str", "opengl32", "uint")
  hGLU := DllCall("LoadLibraryA", "str", "glu32", "uint")
  hGDI := DllCall("LoadLibraryA", "str", "gdi32", "uint")
}
...
{
  global GL_WRAPPER_VERSION
  if (A_IsUnicode)
  {
    if (Lib="glu")
    {
      Proc := DllCall("GetProcAddress", "uint", DllCall("GetModuleHandleW", "wstr", "glu32", "uint"), "astr", FuncName, "uint")
      if (!Proc)
        Proc := DllCall("GetProcAddress", "uint", DllCall("GetModuleHandleW", "wstr", "glu32", "uint"), "astr", FuncName, "uint")
    }
    else if (Lib="gdi")
      Proc := DllCall("GetProcAddress", "uint", DllCall("GetModuleHandleW", "wstr", "gdi32", "uint"), "astr", FuncName, "uint")
    else
      Proc := DllCall("GetProcAddress", "uint", DllCall("GetModuleHandleW", "wstr", "opengl32", "uint"), "astr", FuncName, "uint")
  }
  else
  {
    if (Lib="glu")
    {
      Proc := DllCall("GetProcAddress", "uint", DllCall("GetModuleHandleA", "str", "glu32", "uint"), "str", FuncName, "uint")
      if (!Proc)
        Proc := DllCall("GetProcAddress", "uint", DllCall("GetModuleHandleA", "str", "glu32", "uint"), "str", FuncName, "uint")
    }
    else if (Lib="gdi")
      Proc := DllCall("GetProcAddress", "uint", DllCall("GetModuleHandleA", "str", "gdi32", "uint"), "str", FuncName, "uint")
    else
      Proc := DllCall("GetProcAddress", "uint", DllCall("GetModuleHandleA", "str", "opengl32", "uint"), "str", FuncName, "uint")
  }
  if (!Proc)
  {
    ...
    FileAppend, Crash: %A_Now% (UTC: %A_NowUTC%)`nReason: Unable to load "%FuncName%"!`nWrapper: %GL_WRAPPER_VERSION%`nBit: %bit%`nProc: %Proc%`n`n, %A_ScriptDir%\crash_report.txt
    ExitApp
  }  
  return Proc
}
