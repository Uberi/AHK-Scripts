...
; Code changes:
;
; ID   WHO          YYYY-MM-DD  Description
; ---  -----------  ----------  -----------------------------------------------------------
; @01  HERTERS1893  2007-01-15  Propagate control "label" when "duplicate" is used
; @02  HERTERS1893  2007-01-15  Make "Custom option" show current values in the entry field
; @03  HERTERS1893  2007-01-15  Propagate control "options" when "duplicate" is used
; @04  HERTERS1893  2007-01-15  Handle "extra edit" control on combo box consistently,
;                               and have right click on "extra edit" run on combo box instead
; @05  HERTERS1893  2007-01-15  Put "sleep" into some UI loops, to reduce CPU spinning
; @06  HERTERS1893  2007-01-15  "Tabfound" not zeroed can cause garbage buffer concatenation
; @07  HERTERS1893  2007-01-15  Label info containing newlines is not handled correctly
; @08  HERTERS1893  2007-01-15  Hide deleted controls when tabs are changed
;__________________________________________________________________________________________
...
UI_sleep_interval = 50 ; @05 milliseconds to sleep within some UI loops
...
Duplicate:
	...
	IniRead, CtrlOptions, %Temp%\SGUIControls.ini, %CtrlNameCount%, Options, %A_Space% ; @03
...
CustomOption:
    IniRead, NewOption, %Temp%\SGUIControls.ini, %CtrlNameCount%, Options, %A_Space% ; @02
    ...
    GuiControl, 7:Text, NewOption, %NewOption% ; @02
...
7ButtonOK:
	...
	; @02 IniRead, Options, %Temp%\SGUIControls.ini, %CtrlNameCount%, Options, %A_Space%
	GuiControl, 1:%NewOption%, %CtrlNameCount%
	; @02 Options = %Options% %NewOption%
	Options = %NewOption% ; @02
...
CreateCtrl:
	...
	CtrlText = %Ctrl2Add%
	CtrlOpt_text := "" ; @03
  	IfEqual, JustCopy, Y, SetEnv, CtrlOpt_text, %CtrlOptions% ; @03
	IfEqual, Ctrl2Add, Tab
		CtrlText = Tab1|Tab2
	IfEqual, Ctrl2Add, Tab
		CtrlText = Tab1|Tab2
	...
	IfEqual, Ctrl2Add, Picture
	{
		...
		Gui, 1:Add, %Ctrl2Add%, x%mX% y%mY% %CtrlOpt_text% , %PicFile% ; @03
	}
	...
	IfEqual, Ctrl2Add, Tab
	{
		...
		Gui, 1:Add, %Ctrl2Add%, x%mX% y%mY% w250 h100 vTabName gTabGroup %CtrlOpt_text% , %CtrlText% ; @03
	...
	If Ctrl2Add In Button,Checkbox,ComboBox,DateTime,DropDownList,Edit,GroupBox,ListBox,ListView,MonthCal,Progress,Radio,Slider,Text,UpDown,Hotkey
	{
		...
		IfNotEqual, TabCreated, 1
			Gui, 1:Add, %Ctrl2Add%, x%mX% y%mY% w%cW% h%cH% %CtrlOpt_text% , %CtrlText% ; @03
		Else
			Gui, 1:Add, %Ctrl2Add%, x%mX% y%mY% w%cW% h%cH% %CtrlOpt_text% Border, %CtrlText%  ; @03
    ...
    {   ; @04
		EditCount ++
        combo_edit%editcount% := "ComboBox" %CtrlName%Count ; @04
    }   ; @04
...
; @05		Sleep, 50
	...
	IfNotInString, ItemList, |%CtrlName%%CtrlCount%|
	{
		...
		IniWrite, %CtrlOptions%, %Temp%\SGUIControls.ini, %CtrlName%%CtrlCount%, Options ; @03
	}
...
GenerateGUI:
	...
	IfEqual, SaveGUI, 3
	{
		...
		FinalScript = %FinalScript%Gui`, %GUICountA%Show`, x%wX% y%wY% h%wH% w%wW%`, %Title%`n
	...
	{
		FinalScript = %FinalScript%`; Generated using SmartGUI Creator %Rel%`n
		FinalScript = %FinalScript%Gui`, %GUICountA%Show`, x%wX% y%wY% h%wH% w%wW%`, %GeneratedWnd%`n
...
RightButton:
    ...
    {   ; @04 If cursor on edit control portion of a combobox, set it to the combox box instead of the edit control
        if (substr(combo_%ctrlnamecount%, 1, 8) = "ComboBox")   ; @04
        {                                                       ; @04
            CtrlNameCount := combo_%ctrlnamecount%              ; @04
        }
...
FixText:
	StringReplace, CtrlText, CtrlText, `r,, A ; @07
	StringReplace, CtrlText, CtrlText, `n, ``n, A
	StringReplace, CtrlText, CtrlText, `%, ```%, A
	StringReplace, CtrlText, CtrlText, `;, ```;, A
	StringReplace, CtrlText, CtrlText, `,, ```,, A
...
TabGroup:
    ...
    ; @08 When changing tabs, we have to hide all "DELETED" controls
	tmp_srch := "^DELETED^"                                         ; @08
	tmp_len := strlen(tmp_srch)                                     ; @08
	Loop, Parse, ItemList, |                                        ; @08
	{                                                               ; @08
		IfEqual, A_LoopField,, Continue                             ; @08
		IfInString, A_LoopField, %tmp_srch%                         ; @08
        {                                                           ; @08
		    Buffer := substr(A_LoopField, tmp_len+1)                ; @08
	        Control, hide,, %buffer%, ahk_id %MainWndID%            ; @08
        }                                                           ; @08
    }                                                               ; @08
