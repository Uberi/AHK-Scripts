...
[VERSION]
Set_Version_Info=1
Company_Name=ladiko @ AutoHotkey edited by flashkid
File_Description=AHK Compiler Wrapper
File_Version=0.9.0.55
Inc_File_Version=0
Internal_Name=Compile_AHK.ahk
Legal_Copyright=(c) 2006-2009 AutoHotkey
...
Icon_2=%In_Dir%\AlwaysOnTopOn.ico
Icon_3=%In_Dir%\AlwaysOnTopOff.ico
Icon_4=0
Icon_5=0
Icon_6=0
Icon_7=0
...
; Platform				: WinNT
; Author				: <= 0.9.0.5 @ denick // >= 0.9.0.6 @ ladiko			*edited by flashkid
...
_CHECK_AHK()
{
	...
	If (!Portable_App) {
		...
		FileCopy , %A_WorkingDir%\%Bin_File% , %A_WorkingDir%\%Bin_File%_backup , 1
		If (Errorlevel)
			_Error_Exit(Error_Message . "Couldn't copy """ . A_WorkingDir . "\" . Bin_File . """ to """ . A_WorkingDir . "_backup"".")
	}
	Return
}
; --------------------------------------------------------------------------------
...
; --------------------------------------------------------------------------------
; Save Ini Settings in Script File
; --------------------------------------------------------------------------------
_SaveIniInScript()
{
	Global
	Local s_Creation , s_Modification , s_Access , In_File_Content , Ini_File_Content
	...
	RegExMatch(In_File_Content, "is)Compile_AHK SETTINGS BEGIN \* \* \*\n\n(.+?)\n\* \* \* Compile_AHK SETTINGS END", In_File_Settings)
	...
	If (In_File_Settings1= Ini_File_Content)	; settings match old settinsg
		Return
	...
	In_File_Content := "/*" . new_line . " * * * Compile_AHK SETTINGS BEGIN * * *" . new_line . new_line
		. Ini_File_Content
		. new_line . "* * * Compile_AHK SETTINGS END * * *" . new_line . "*/" . new_line . new_line
		. In_File_Content
	
	FileAppend , %In_File_Content% , %In_File%
	
	; to notify editors about changed file content
	EnvAdd , s_Modification , 1 , Seconds
	EnvAdd , s_Access , 1 , Seconds
	
	FileSetTime , s_Creation , %In_File% , C
	FileSetTime , s_Modification , %In_File%, M
	FileSetTime , s_Access , %In_File% , A
...
_Get_Gui_Values()
{
	...
	SaveIniInScript := Gui_SaveIniInScript
...
Set_UPX_Option()
{
	...
	s_UPX_File := A_WorkingDir . "\" . UPX_Exe
	Error_Message := "NoUPX Error.`n"
	
	If (!Portable_App) {
		FileAppend , `n* Set No_UPX option`nFileDelete `, %s_UPX_File% , %A_WorkingDir%\%Log_File%
		
		FileDelete , %s_UPX_File%
		If Errorlevel
			_Error_Exit(Error_Message . "Couldn't delete " . s_UPX_File)
			
		FileAppend , `n# Set No_UPX option successfully`n , %A_WorkingDir%\%Log_File%
	} else {
		FileAppend , `n* Set No_UPX option`nFileMove `, %s_UPX_File% `, %s_UPX_File%_backup , %A_WorkingDir%\%Log_File%
		
		FileMove , %s_UPX_File% , %s_UPX_File%_backup
		If Errorlevel
			_Error_Exit(Error_Message . "Couldn't move """ s_UPX_File . """ to """ . s_UPX_File . "_backup"".")
...
Restore_File(File) {
	...
	Error_Message := "Restore Error!`n"
	IfExist , %File%_backup
	{
		FileAppend , `n* Restore No_UPX option`nFileMove `, %File%_backup `, %File% , %A_WorkingDir%\%Log_File%
		FileMove , %File%_backup , %File% , 1
		If ErrorLevel
			_Error_Exit(Error_Message . "Couldn't copy """ . File . "_backup"" to """ . File """.")
		FileAppend , `n# Restored No_UPX option successfully`n , %A_WorkingDir%\%Log_File%
	}
	Return
}
; --------------------------------------------------------------------------------
; Clean Registry: HKCU\Software\AutoHotkey on portable installations
; --------------------------------------------------------------------------------
Clean_Registry() {
	...
	Error_Message := "Registry Error!`n"
	
	RegDelete, HKCU, Software\AutoHotkey
	If ErrorLevel
		_Error_Exit(Error_Message . "Couldn't delete ""HKCU\Software\AutoHotkey"".")
	Return
}
; --------------------------------------------------------------------------------
; Delete Temporary files
...
updateLang:
	...
	GuiControl , , Gui_SaveIniInScript , %Lang_SaveIniInScript%
