...
Keys:
   ctrl & l --- runs the program on th active window.
   esc      --- Exit
...
Notes:
   ...
   * The output is in .obj (and .mtl) format. A standard 3D format.
   * This does nothing advanced, don't expect to much.
   * The program exits after saving the files.
   * Some controls and windows comes out black, I do not know why. I blame GDI+ and/or the window itself.
...
; fill:  0xAARRGGBB format. This is used for Per-control texture mode. if non-zero or blank, the base image will have a rectangle overlaying the controls.
PerControlImage:=1
Fill=0xff7cb083
; Fill=   ; using any of these 2 will show the controls on the Base Window image.
; Fill=0  ; using any of these 2 will show the controls on the Base Window image.
...
if (!FileExist(A_ScriptDir "\" filesFolder))
   FileCreateDir, %A_ScriptDir%\%filesFolder%
...
^l::
   ...
   if (PerControlImage=0) ; use the same image for everything.
      materials.=materials("Base_Window", image, ID, 0, rnd(0.5, 1.0), rnd(0.5, 1.0), rnd(0.5, 1.0), fill)
   else ; use the image as defined by the rect() function above
      materials.=materials("Base_Window", "Base_Window", ID, 0, rnd(0.5, 1.0), rnd(0.5, 1.0), rnd(0.5, 1.0), fill)
   ...
   Loop, Parse, Controls, `n, `r
   {
      ...
      if (PerControlImage=0) ; use the same image for everything.
         materials.=materials(A_LoopField, image, ID, PerControlImage, rnd(0.5, 1.0), rnd(0.5, 1.0), rnd(0.5, 1.0), fill)
      else ; use the image with the same name as the control.
         materials.=materials(A_LoopField, A_LoopField, ID, PerControlImage, rnd(0.5, 1.0), rnd(0.5, 1.0), rnd(0.5, 1.0), fill)
   }
   ...
   {
      ToolTip deleting
      FileDelete, %A_ScriptDir%\%filesFolder%\%filename%.obj
      FileDelete, %A_ScriptDir%\%filesFolder%\%filename%.mtl
      sleep, 200
      ToolTip saving
      sleep, 400
      FileAppend, %header%`n%vertcoords%, %A_ScriptDir%\%filesFolder%\%filename%.obj
      FileAppend, %materials%, %A_ScriptDir%\%filesFolder%\%filename%.mtl
   }
...
; materials(name, image, win, mode=0, r=0, g=0, b=0, fill="0xff7cb083")
...
materials(name, image, win, mode=0, r=0, g=0, b=0, fill="0xff7cb083")
{
   ...
   {
      pBitmap := Gdip_BitmapFromHWND(win)    ; get a pointer/bitmap of the window.
      
      if (fill!="" || fill!=0)
      {
         G := Gdip_GraphicsFromImage(pBitmap)   ; now we convert it so we can draw on it.
         pBrush := Gdip_BrushCreateSolid(fill)   ; then we make the brush for our doodle.
         GetWinData("ahk_id " win, ClientX, ClientY, ClientW, ClientH, BorderW, BorderH, CaptionH)
         Gdip_FillRectangle(G, pBrush, ClientX, ClientY, ClientW, ClientH) ; then we do the doodling.
         Gdip_DeleteBrush(pBrush)
         Gdip_DeleteGraphics(G)
      }

      ; save the image.
      Gdip_SaveBitmapToFile(pBitmap, A_ScriptDir "\" filesFolder "\" image ".jpg")
   ...
   Else if (mode=1)
   {
      ...
      Gdip_SaveBitmapToFile(pBitmap, A_ScriptDir "\" filesFolder "\" image ".jpg")
      Gdip_DeleteGraphics(pBitmap)
      ; offset:=0 ; reset it, just incase (which could be never, but whatever)
   }
   Else
      return "error"

   ; blah=newmtl %name%`nKd %r% %g% %b%`nmap_Kd %A_ScriptDir%\%filesFolder%\%image%.jpg`n`n
   Gdip_DisposeImage(pBitmap)
   return "newmtl " name "`nKd " r " " g " " b "`nmap_Kd " image ".jpg`n`n" ; file paths are relative to the .obj file.
}
...
Rect(x,y,z,w,h, mode=0, comment="", name="")
{
   ...
   coords.="f " 4+offset "/" 4+offset "/" 1 " " 3+offset "/" 3+offset "/" 1 " " 2+offset "/" 2+offset "/" 1 " " 1+offset "/" 1+offset "/" 1 "`n`n"
...
GetWinData(hwnd, ByRef ClientX, ByRef ClientY
               , ByRef ClientW, ByRef ClientH
               , ByRef BorderW, ByRef BorderH
               , ByRef CaptionH)
{
   WinGet, hwnd, ID, %hwnd%
   SysGet, BorderW,   7
