...
#SingleInstance, force
    ...
    Gui, SHow, AutoSize, ScriptMerge    by majkinetor 
...
_OnButtonClick:
	...
	if (operation = "join")
	{
		...
		If !InStr( fn, "!J!.ahk") 	{
			output = File name must end with !J!.ahk`n `nIf you are sure that this file is joined using`nJoin function rename it and try again
			goto end
		}
		
		text := Unjoin(fn, output)

		StringReplace, fn, fn, %A_SPACE%!J!
		FileDelete, %fn%
		FileAppend %text%, %fn%
...
Unjoin( pFileName, ByRef pCreated, firstCall=1 ) {
	...
	static reE := ";=+[ ]END(?:\1)?[:][ ]\2:B1AD529B-BF4E-477F-8B9F-3080CAC55AE3`r`n"
	static sDir
	re = is)%reS%\R(.+?)\R%reE%			; 1 - DIR
...
Join( pFileName, ByRef pIncluded, firstCall=1){
	static line := ";==================== X :B1AD529B-BF4E-477F-8B9F-3080CAC55AE3`r`n"
	static sDir, sDrive
...
;	pIncluded .= pFileName "`n"								; Add this file to include list so not to add it again
	...
	;check is it dir   
		...
		if InStr(attrib, "D") {
			...
			rep  := RegExReplace(line,"X", "START DIR: " out) . "`r`n" . RegExReplace(line, "X", "END DIR: " out)
	...
	;is it already included ?
		if !InStr( pIncluded, out1) 	{
			 ...
			 rep := RegExReplace(line,"X", "START: " out)
				  . Join( incPath, pIncluded, false) . "`r`n"  
