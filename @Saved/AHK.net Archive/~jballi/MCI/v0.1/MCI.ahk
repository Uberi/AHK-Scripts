...
;*********************
/*
Each MCI function is documented separately.  The following (currently
unstructured) is random information that relates to the entire library:
...
;   Description:
;
;       Opens a MCI device and loads the specified file (p_MediaFile).
;
;
;   Parameters:
;
...
;
;                  16-bit: HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows NT\CurrentVersion\MCI
;                  32-bit: HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows NT\CurrentVersion\MCI32
;
...
;               command string, go to the "MCI Reference" section of the msdn
;               web site.  There should be a link at the top of this library.
;
;
;   Return value:
;
...
MCI_Open(p_MediaFile,p_Alias="",p_Flags="")
    ...
    IfNotExist %p_MediaFile%
        ...
        outputdebug,
           (ltrim join`s
            End Func: %A_ThisFunc%: The media file can't be found.  Return=0
           )
    ...
    if InStr(A_Space . p_Flags . A_Space," type ")=0
        ...
        if ErrorLevel
            ...
            outputdebug,
               (ltrim join`s
                End Func: %A_ThisFunc%: The file extension for this media file
                is not registered as a valid MCI extension.  Return=0
               )
...
MCI_Close(p_lpszDeviceID)
    ...
    if OnMessage(MM_MCINOTIFY)="MCI_Notify"
        ...
        if l_OpenMCIDevices=0
            OnMessage(MM_MCINOTIFY,"")  ;-- Disable monitoring
        }
...
;           is critical.  If not defined correctly, the script will crash.
;
...
;           [Optional].  If undefined but needed, the handle to first window
;           found for the process that currently running is used.
;
;
;   Return value:
;
...
MCI_Play(p_lpszDeviceID,p_Flags="",p_Callback="",p_hwndCallback=0)
    ...
    if not p_hwndCallback
        ...
        or StrLen(p_Callback)
            ...
            Process Exist
            p_hwndCallback:=WinExist("ahk_pid " . ErrorLevel)
...
;   Remarks:
;
;       Per msdn: With the MCICDA, MCISEQ, and MCIPIONR drivers, the pause
;       command works the same as the stop command.
;
...
;   Remarks:
;
;       Per msdn: Digital-video, VCR, and waveform-audio devices recognize this
...
;   Description:
;
;       Move to a specified position and stop.
;
;
;   Parameters:
;
;       p_lpszDeviceID - Device name or alias. [Required]
;
;       p_Position - Position to stop the seek. [Required]
;
;       p_Track - Track number. [Optional]  The default is 0 (no track).
;
;
;   Return value:
;
...
Follow up:
Code to support cdaudio left in place but may not be not used because of bugs is
the default MCI code or in the CDAudio devices.
*/


MCI_Seek(p_lpszDeviceID,p_Position,p_Track=0)
    {
    ;-- Adjust p_Position
    if p_Position is Number
    ...
    if p_Track
        l_TrackFromPos:=MCI_Position(p_lpszDeviceID,p_Track)
            ;-- This collects the starting position of the track

    l_CurrentPos:=MCI_Position(p_lpszDeviceID)
        ;-- This collects the current position of the device relative to the
        ;   beginning of the entire media


    l_Length:=MCI_Length(p_lpszDeviceID,p_Track)
        ;-- Length of the track or entire media



    ;-- Adjust position if necessary
    l_TargetPos:=l_TrackFromPos+p_Position

    if (l_TargetPos>(l_TrackFromPos+l_Length))
        l_TargetPos:=l_TrackFromPos+l_Length

    if l_TargetPos<1
        if p_Track
            l_TargetPos:=l_TrackFromPos
         else
            l_TargetPos:="start"
                ;-- This is necessary because some devices do no like a "0"
                ;   position.


    ;-- Seek
    l_CmdString=seek %p_lpszDeviceID% to %l_TargetPos% wait
    l_Return:=MCI_SendString(l_CmdString,Dummy)
...
;
;       Per msdn: All devices can return the "not ready", "paused", "playing",
;       and "stopped" values. Some devices can return the additional "open",
...
;       Per msdn: Sequencer, video-overlay, digital-video, and waveform-audio
...
;   Remarks:
;
;       Per msdn: The MCISEQ and MCIWAVE devices return 1, as do most VCR
;       devices. The MCIPIONR device does not support this flag.
;
;------------------------------------------------------------------------------
