; ------------------------------------------------------------------------------
; AutoHotkey Version:  1.0.46.08
; Language:            English
; Platform:            WinXP
; Author:              denick
; Version:             0.90/2006-11-11/denick
; Script Function:     Gui für AHK2EXE.EXE
;
; Das Script wertet eine im Scriptverzeichnis vorhandene scriptname.ahk.ini
; aus und steuert damit den Compiler- und ggf. ResHack-Aufruf. Ggf. wird die
; ini-Datei mit Defaultwerten neu angelegt.
...
BIN_FILE := A_ScriptDir . "\AutoHotkeySC.bin"
RC_EXE := A_ScriptDir . "\GoRC.exe"
RES_EXE := A_ScriptDir . "\ResHacker.exe"
RES_INI := A_ScriptDir . "\ResHacker.ini"
RES_LOG := A_ScriptDir . "\ResHacker.log"
LOG_FILE := A_ScriptDir . "\Compile_AHK.log"
AHK_SECTION := "AHK2EXE"
RES_SECTION := "VERSION"
MSG_TITLE := "Compile AHK"
COMPR_LEVELS := "Lowest|Low|Normal|High|Highest"
UPX_SETTINGS := " --best --filter=73 --lzma --compress-icons=0 "
; ------------------------------------------------------------------------------
; Variable
; ------------------------------------------------------------------------------
; Compiler settings
...
If (!FileExist(COMPILER)) {
   _ERROR_EXIT(ERR_MSG . COMPILER . " not found")
}
If (!FileExist(BIN_FILE)) {
   _ERROR_EXIT(ERR_MSG . BIN_FILE . " not found")
}
_CHECK_AHK_BIN()
; ------------------------------------------------------------------------------
...
Gui, Add, Tab, Choose1 x0 y20 w615 h415, Compiler Options|Version Info
Gui, Tab, 1
...
Gui, Add, Button, Default xs y390 h25 w70 gGuiBTCompile, Compile
Gui, Add, Button, x+430 yp h25 w70 gGuiClose, Abort
Gui, Tab, 2
Gui, Add, Checkbox, x+20 y+20 gGUICBVI vGUI_VERSION_INFO, Set Version Info
Gui, Font, cNavy
...
GuiControl, , GUI_VERSION_INFO, %VERSION_INFO%
If (!VERSION_INFO) {
...
; FUNCTIONS SECTION ============================================================
; ==============================================================================
; Check AutoHotkeySC.bin
; ------------------------------------------------------------------------------
_CHECK_AHK_BIN()
{
   Global
   Local s_AHK, s_Bin, s_Org, a_V
   If (!FileExist(BIN_FILE . ".org")) {
      FileCopy, %BIN_FILE%, %BIN_FILE%.org
      If (Errorlevel <> 0) {
         _ERROR_EXIT(ERR_MSG . "Couldn't save " . BIN_FILE)
      }
      Return
   }
   FileGetVersion, s_Bin, %BIN_FILE%
   FileGetVersion, s_Org, %BIN_FILE%.org
   FileGetVersion, s_AHK, ..\AutoHotkey.exe
   StringSplit, a_V, s_AHK, .
   If (s_Org = s_AHK) {
      Return
   }
   Else If (s_Bin = s_AHK) {
      FileCopy, %BIN_FILE%, %BIN_FILE%.org, 1
      If (Errorlevel <> 0) {
         _ERROR_EXIT(ERR_MSG . "Couldn't save " . BIN_FILE)
      }
   }
   Else
   {
      _ERROR_EXIT(ERR_MSG . "AHK Version: " s_AHK "`nBIN Version: " s_BIN)
   }
   Return
}
; ------------------------------------------------------------------------------
...
_SET_VERSION()
{
   ...
   FileCopy, %BIN_FILE%.org, %BIN_FILE%, 1
   If (Errorlevel <> 0)
   {
      _ERROR_EXIT(ERR_MSG . "Couldn't restore " . BIN_FILE)
   }
