...
AhkDllThread_IsH(){
   static CS := A_IsUnicode ? "W" : "A",AHK:=A_AhkPath?A_AhkPath:A_ScriptFullPath
   FSz:=DllCall("Version\GetFileVersionInfoSize" CS, "Str",AHK, "UInt",0 )
   VarSetCapacity( FVI, FSz, 0 ),VarSetCapacity( Trans,8*(A_IsUnicode?2:1) )
   DllCall("Version\GetFileVersionInfo" CS, "Str",AHK, "Int",0, "UInt",FSz, "UInt",&FVI )
   ...
   subBlock := "\StringFileInfo\" SubStr(Trans,-4) SubStr(Trans,1,4) "\InternalName"
   DllCall("Version\VerQueryValue" CS, "UInt",&FVI, "Str",SubBlock, "UIntP",InfoPtr, "UInt",0 )
   Return StrGet( InfoPtr,DllCall("lstrlenW","PTR",InfoPtr) ) = "AutoHotkey_H" 
			 || StrGet( InfoPtr,DllCall("lstrlenW","PTR",InfoPtr) ) = " "
}
...
AhkDllThreadDLL(dll:=`"AutoHotkey.dll`",obj:=0){
  ...
  LoopParse,`%functions`%,|
  {
    ...
    object[map=`"`" ? v.1 : !InStr(map,v.1) ? v.1 : SubStr(map,InStr(map,v.1)+StrLen(v.1)+1,InStr(map,A_Space,0,InStr(map,v.1)))]:=DynaCall(object[`"`"].GetProcAddress(v.1),v.2)
  }
  ...
  {
    StrSplit,v,%A_LoopField%,:
    object[map="" ? v.1 : !InStr(map,v.1) ? v.1 : SubStr(map,InStr(map,v.1)+StrLen(v.1)+1,InStr(map,A_Space,0,InStr(map,v.1)))]:=%DynaCall%(%MemoryGetProcAddress%(object[""],v.1),v.2)
  }
  object.base:=base
  return object
}
