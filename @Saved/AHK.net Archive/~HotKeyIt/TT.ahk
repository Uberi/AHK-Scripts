...
TT(options="",text="",title=""){
  ...
  ; Objects
  static @:={},base:={Color:"TT_Color",Show:"TT_Show",Hide:"TTM_Trackactivate",Close:"TT_Close",Add:"TT_Add",AddTool:"TTM_AddTool"
...
TT_Remove(T=""){
  static @:=TT(-0x8000000000000000) ;Get main object that holds all ToolTips
  If !T
    Loop % @.MaxIndex()
    {
      T:=@[A_Index]
      ...
      for id,tool in T.T
        T.DelTool(tool[])
      T.DelTool(T.P[])
      DllCall("DestroyWindow","PTR",T.HWND)
    }
  else
    for id,Tool in @
    {
...
TT_Icon(T,icon=0,icon#=1){
  ...
  If icon
   ...
   Return DllCall("SendMessage","PTR",T.HWND,"UInt",TTM_SETTITLE,"PTR",icon,"PTR",T.GetAddress("maintitle"),"PTR"),T.UPDATE()
}
...
TT_GetIcon(File="",Icon#=1){
  ...
  If !File {
    for file,obj in hIcon
      If IsObject(obj){
        ...
        DllCall("DestroyIcon","PTR",handle)
    DllCall("gdiplus\GdiplusShutdown", "PTR",pToken),sfi:=""
    Return
  }
  If (SubStr(file,-3)=".lnk")
  {
     FileGetShortcut,%File%,Fileto,,,,FileIcon,FileIcon#
     File :=FileIcon ?FileIcon :Fileto
     Icon#:=FileIcon#?FileIcon#:Icon#
  }
  If hIcon[File,Icon#]
  ...
  If (ext="cur")
  {
    Return hIcon[file,Icon#]:=DllCall("LoadImage", "PTR", 0, "str", File, "uint", ext="cur"?2:1, "int", 0, "int", 0, "uint", 0x10,"PTR")
  } else if InStr(",EXE,ICO,DLL,LNK,","," Ext ",")
  {
    DllCall("PrivateExtractIcons", "Str", File, "Int", Icon#-1, "Int", SmallIconSize, "Int", SmallIconSize, "PTR*", Icon, "PTR*", 0, "UInt", 1, "UInt", 0, "Int")
...
TT_Show(T,text="",x="",y="",title="",icon="",icon#=1){
  ...
  If (x="TrayIcon" || y="TrayIcon"){
    ...
    bufAdr  := DllCall( "VirtualAllocEx", "PTR", DataH, "PTR", 0, "uint", sizeof(TB), "uint", MEM_COMMIT:=0x1000, "uint", PAGE_READWRITE:=0x4,"PTR")
    max:=DllCall("SendMessage","PTR",hWndToolBar,"UInt",0x418,"PTR",0,"PTR",0,"PTR")
    Loop % max
    {
      i:=max-A_Index
  ...
  T.TRACKPOSITION(x="Caret"?A_CaretX:x,y="Caret"?A_CaretY:y)
  T.TRACKACTIVATE(1)
}
...
TTM_SETTITLE(T,title="",icon="",Icon#=1){
  ...
  If Icon!=
    lastIcon:=Icon
   Return DllCall("SendMessage","PTR",T.HWND,"UInt",TTM_SETTITLE,"PTR",icon,"PTR",T.GetAddress("maintitle"),"PTR"),T.UPDATE(),lastIcon:=Icon
}
