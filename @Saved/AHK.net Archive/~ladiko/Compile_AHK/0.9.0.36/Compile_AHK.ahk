...
GuiShow:
	...
	Gui , Tab , 2
	Gui , Add , Checkbox , x+20 y+10 gGui_CB_VI w150 vGui_Set_Version_Info Checked%Set_Version_Info% , %Lang_Set_Version_Info%
...
ShowCredits:
	...
	Gui 2:Add, Picture, vIcon7 x12 y%YPos8% h32 w32 Icon100, shell32.dll
	Gui 2:Add, Text, vURL_7 gOpenLink x48, Language Helper by haichen
	
	Gui 2:Add, Picture, vIcon8 x12 y%YPos9% h32 w32 Icon14, shell32.dll
	Gui 2:Add, Text, vURL_8 gOpenLink x48, Help by The Community
	...
	Loop
	{
		...
		YPos9--
		GuiControl 2:Move, Icon8, y%YPos9%
		GuiControl 2:Move, URL_8, y%YPos9%
		YPos10--
...
_READ_INI(s_INI_FILE)
{
	...
	If (s_INI != "ERROR") {
		If s_INI Not In 0,1
		{
			_ERROR_EXIT(ERR_MSG . "Show_Pwd = " . s_INI)
			Return
		}
		Show_Pwd := s_INI
	}
	...
	If (s_INI != "ERROR") {
		If s_INI Not In 0,1
		{
			_ERROR_EXIT(ERR_MSG . "NoDecompile = " . s_INI)
			Return
		}
		NoDecompile := s_INI
	}
	...
	If s_INI Not In 0,1
	{
		_ERROR_EXIT(ERR_MSG . "Set_Version_Info = " . s_INI)
		Return
	}
...
; --------------------------------------------------------------------------------
ReadLanguage:
	IfExist , %USER_LANGUAGE_INI%
	{
		FileGetTime , USER_LANG_TIME , %USER_LANGUAGE_INI%
		FileGetTime , GLOBAL_LANG_TIME , %GLOBAL_LANGUAGE_INI%
		If (GLOBAL_LANG_TIME > USER_LANG_TIME)
		{
			SplitPath , USER_LANGUAGE_INI , , USER_LANGUAGE_DIR
			
			IniRead , _actualLang , %USER_LANGUAGE_INI% , Lang , actualLang , en-us
			
			FileCopy , %GLOBAL_LANGUAGE_INI% , %USER_LANGUAGE_INI% , 1
			If ErrorLevel
				_ERROR(ERR_MSG . "`nCould not copy """ . GLOBAL_LANGUAGE_INI . """ to """ . USER_LANGUAGE_DIR . """.")
			
			IniWrite , %_actualLang% , %USER_LANGUAGE_INI% , Lang , actualLang
			If ErrorLevel
				_ERROR(ERR_MSG . "`nCould not write to """ . USER_LANGUAGE_INI . """.")
		}

	} Else {
		SplitPath , USER_LANGUAGE_INI , , USER_LANGUAGE_DIR
		IfNotExist , %USER_LANGUAGE_DIR%
		{
			FileCreateDir , %USER_LANGUAGE_DIR%
			If ErrorLevel
				_ERROR(ERR_MSG . "`nCould not create the folder """ . USER_LANGUAGE_DIR . """")
		}
	...
	FileRead, lang_file, %USER_LANGUAGE_INI%
...
updateLang:
	...
	IniWrite , %actualLang% , %USER_LANGUAGE_INI% , Lang , actualLang
	If ErrorLevel
			_ERROR(ERR_MSG . "`nCould not write to """ . USER_LANGUAGE_INI . """.")
return
; --------------------------------------------------------------------------------
...
CommandLine:
	...
	If (In_File = "")
		_ERROR_EXIT(ERR_MSG . "`nNo file Selected.")
