...
{
	Global
	Local s_CMD , s_ERR , s_LOG , s_MANIFEST , XML_POS , s_LEFT , s_RIGHT , s_SC_File , s_MF_FILE
...
; --------------------------------------------------------------------------------
	...
	FileAppend , `n* Delete original Versioninfo (ResHacker-fix for non-1033 systems):`n%s_CMD%`n, %A_WorkingDir%\%LOG_FILE%
	RunWait , %ComSpec% /c "%s_CMD%" , , UseErrorLevel Hide
	If (Errorlevel)
	{
		FileRead , s_Log , %A_WorkingDir%\%RES_LOG%
	...
	FileDelete , %A_WorkingDir%\%RES_LOG%
	FileAppend , `n~~ Reshacker Log Start ~~`n%s_Log%~~ Reshacker Log End ~~`n`n# Versioninfo deleted successfully!`n`n , %A_WorkingDir%\%LOG_FILE%
