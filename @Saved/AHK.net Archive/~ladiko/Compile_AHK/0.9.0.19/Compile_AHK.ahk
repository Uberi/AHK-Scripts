...
Gui , Add , Button , xm+554 ym h23 w60 gShowCredits , Credits
...
_CHECK_AHK()
{
	...
	If (s_OrgComp <> s_UsedComp) {
		FileCopy , %AHK_DIR%\%COMPILER% , %A_WorkingDir%\%COMPILER% , 1
		If (Errorlevel) {
	...
	If (s_OrgUPX <> s_UsedUPX) {
		FileCopy , %AHK_DIR%\%UPX_EXE% , %A_WorkingDir%\%UPX_EXE% , 1
		If (Errorlevel) {
			_ERROR_EXIT(ERR_MSG . "Couldn't copy new " . UPX_EXE)
		}
	}
	FileCopy , %AHK_DIR%\%BIN_FILE% , %A_WorkingDir%\%BIN_FILE% , 1
	If (Errorlevel) {
