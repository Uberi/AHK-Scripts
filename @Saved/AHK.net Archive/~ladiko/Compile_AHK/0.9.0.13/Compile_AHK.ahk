...
If(FileExist(A_ScriptDir . "`\AllOnTopOn.ico") && FileExist(A_ScriptDir . "`\AllOnTopOff.ico"))
{
	OnTopIcons = 1
	Gui, Add, Picture, xm+494 ym+4 h16 w16 hidden vGuiOnTop gGuiOnTop_OFF AltSubmit, %A_ScriptDir%\AllOnTopOn.ico
	Gui, Add, Picture, xm+494 ym+4 h16 w16 vGuiOnTop_OFF gGuiOnTop AltSubmit, %A_ScriptDir%\AllOnTopOff.ico
}
Else
{
	OnTopIcons = 0
...
GuiOnTop:
	...
	If (OnTopIcons)
	{
		ToolTip, AllwaysOnTop is On, 525, 45
...
GuiOnTop_OFF:
	...
	If (OnTopIcons)
	{
		ToolTip, AllwaysOnTop is Off, 525, 45
		SetTimer, RemoveToolTip, 1500
	}
...
GuiBTSelIcon:
	...
	If (TempIcon = "")
		TempIcon = %A_WinDir%\system32\shell32.dll
	nIndex := 0

	WinGet, hWnd, ID, PickIconDlg

	VarSetCapacity(wTempIcon, 260 * 2)
	DllCall("MultiByteToWideChar", "Uint", 0, "Uint", 0, "str", TempIcon, "int", -1, "str", wTempIcon, "int", 260)

	DllCall("shell32\PickIconDlg", "Uint", hWnd, "str", wTempIcon, "Uint", 260, "intP", nIndex)

	VarSetCapacity(TempIcon, 260)
	DllCall("WideCharToMultiByte", "Uint", 0, "Uint", 0, "str", wTempIcon, "int", -1, "str", TempIcon, "int", 260, "Uint", 0, "Uint", 0)
	
	StringReplace , TempIcon , TempIcon , `%SystemRoot`% , %A_WinDir%
	
	; ResHacker uses ResourceIds instead of IconIndex
	IconGroupID := ResourceIdOfIcon(TempIcon, nIndex)

	SplitPath , TempIcon , , , TempIconExt
	...
	If (TempIconExt != "ICO")
	{
		...
		If (Errorlevel)
		{
			...
			s_ERR := "Couldn't extract icon %ICONINDEX% from %TempIcon%, " . RES_EXE . " failed!`n" . s_LOG
