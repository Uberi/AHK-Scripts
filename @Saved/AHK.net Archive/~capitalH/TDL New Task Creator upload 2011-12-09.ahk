...
Settings:=Object()
;Settings.Checkout:=1   ;Automatic checkout
Settings["Columns"]:=2
Settings.DoubleFile:=1
;Visibility settings
Settings.Priority:=0
Settings.Risk:=0
Settings.DueDate:=3
Settings.StartDate:=3
Settings.CreationDate:=3
Settings.LastModDate:=3
Settings.Cost:=0
Settings.TimeEst:=0
Settings.TimeDue:=0
...
Width2=140
ButtonWidth=24
ListViewWidth:=Width2+ButtonWidth+2
XM:=5
ColDX1:=XM
ColW1:=50
ColW2:=140
ColVX1:=ColDX1+ColW1+XM
YPOffSet:=3

loop, % Settings.Columns ;%
{
	if a_index>1
	{
		tmp:=A_Index-1
		ColDX%A_Index%:=ColVX%tmp%+ColW2+XM
		ColVX%A_Index%:=ColDX%A_Index%+ColW1+XM
	}
}
...
^N::
WinGet, PathToTDL , ProcessPath , AbstractSpoon
SendMessage, 0x111, 32799  				;create new task
clipboardsave:=clipboardall 			;save clipboard
SendMessage, 0x111, 33193  				;copy selected task as text
clipwait								;wait for clipboard to change
TaskID:=clipboard						;extract clipboard test
Clipboard:=clipboardsave				;restore clipboard
if instr(TaskID,"(ID: ")   				;Copy success! Get the task ID of the new task
{
	taskID:=substr(TaskID, instr(taskID,"(ID: ")+5)
	taskID:=substr(TaskID, 1, instr(taskID,")")-1)	
}
else
	return  			;cannot find ID
SendMessage, 0x111, 32816
Gui, Destroy

StatusBarGetText, Filename , 1, AbstractSpoon
if instr(Filename,".tdl (Unicode)")
	Stringreplace,Filename,Filename, (Unicode)

FileRead, xml, %Filename%
TDL:=loadXML(XML)
Categories:=TDLCategories(TDL, "Category") 
AllocatedToList:=TDLCategories(TDL, "PERSON") 
...
CurCol=1
Gui, Add, Text, xm ym w0,
DropDownList10=<None>|0 (Lowest)|1 (Very Low)|2 (Low) |3 ((Low-Med)|4 (Medium)|5 (Medium)|6 (Medium)|7 (Med-High)|8 (High)|9 (Very High)|10 (Highest)
GuiAddEdit("TDL Filename", "FileName", Filename ,X2,Width2)
GuiAddEdit("Title", "Title", "Task" ,X2,Width2)
if(Settings.Priority) 
	GuiAddDropDown("Priority","Priority","Choose1",X2,Width2,DropDownList10)
if (Settings.Risk)	
	GuiAddDropDown("Risk","Risk","Choose1",X2,Width2,DropDownList10)
if (Settings.Cost)	
	GuiAddEdit("Cost", "Cost", "0.0" ,X2,Width2,1)
TimeEstUnits=H
TimeSpentUnits=H
if (Settings.TimeEst)
	GuiAddEditTime("Time Est.", "TimeEstimate", "0.0" ,X2,Width2,TimeEstUnits,"TimeEstUnitSelector") ;Label TimeEstUnitSelector handles time unit selection
if (Settings.TimeSpent)
	GuiAddEditTime("Time Spent", "TimeSpent", "0.0" ,X2,Width2,TimeSpentUnits,"TimeSpentUnitSelector") ;Label TimeSpentUnitSelector handles time unit selection
if (Settings.DueDate)
	GuiAddDate("Due Date", "DueDate", "0.0" ,X2,Width2,Settings.DueDate,"DueTime")
if (Settings.StartDate)
	GuiAddDate("Start Date", "StartDate", "0.0" ,X2,Width2,Settings.StartDate,"StartTime")
if (Settings.LastModDate)
	GuiAddDate("Last Modification", "LastModDate", "0.0" ,X2,Width2,Settings.LastModDate,"LastModTime")
if (Settings.CreationDate)
	{
		GuiAddDate("Creation Date", "CreationDate", "0.0" ,X2,Width2,Settings.CreationDate,"CreationTime")
		GuiControl,,CreationDate, %A_Now%
		FormatTime, CreationTime , %A_Now%,HH:mm
		GuiControl,,CreationTime, %CreationTime%||
	}

;File link code begin
;This code is unique - though to clean up this section should consider moving to a function
If (Settings.DoubleFile) and (settings.columns>=2)
{
	FWidth:=2*ColW2+ColW1+2*XM-2*ButtonWidth
	if (CurCol+1>Settings.Columns)
	{	
		CurCol:=1
		GetX:=ColDX%CurCol%
		Gui, Add, Text, x%GetX% y+10 , File link
		GetX:=ColVX%CurCol%
		Gui, Add, Edit, x%GetX% yp-%YPOffset% w%FWidth% vFileLink 
	}
		else
	{
		GetX:=ColDX%CurCol%
		Gui, Add, Text, x%GetX% yp+%YPOffset% , File link
		GetX:=ColVX%CurCol%
		Gui, Add, Edit, x%GetX% yp-%YPOffset% w%FWidth% vFileLink 
	}
	...
	Gui, Font, ,Wingdings
	Gui, Font, s10
	Gui, Add, Button,  gFileLinkSelect x+0 w%ButtonWidth% h22 0x8000, 1
	Gui, Add, Button,  gFileLinkOpen x+0 w%ButtonWidth% h22 0x8000, $
	Gui, Font, 
	CurCol++
	CurCol++
}
...
{
	FWidth:=ColW2-2*ButtonWidth
	if (CurCol>Settings.Columns)
	{	
		CurCol:=1
		GetX:=ColDX%CurCol%
		Gui, Add, Text, x%GetX% y+10 , File link
...
Gui, Add, Edit, x%X2% w%Width2% yp vCategory  gCategory
Gui, Font, ,Webdings
Gui, Font, s16
Gui, Add, Button, HWNDCategoryBtn x+0 gCategoryLVLabel w%ButtonWidth% h22 0x8000, 6
Gui, Font, 

Gui, Add, Text, xm y+10 , Allocated To
Gui, Add, Edit, x%X2% w%Width2% yp vAllocatedTo  gAllocatedTo
Gui, Font, ,Webdings
Gui, Font, s16
Gui, Add, Button, HWNDAllocatedToBtn x+0 gAllocatedToLVLabel w%ButtonWidth% h22 0x8000, 6
Gui, Font, 

Gui, Add, Button, xm y+10 gSave default,Save
Gui, Add, Button, x+10 yp gCancel,Cancel
Gui, Add, Button, x+10 yp gOptions, Options

Gui, Add, ListView, Checked HWNDAllocatedToLV w%ListViewWidth% x20 y28 R10  -Hdr vComboBox1, Font  ;h190
FillAllocatedTo(TDL)
AddRowLV1=0
ComboX_Set( AllocatedToLV, "4LD esc space enter" AllocatedToBtn+0, "OnComboAllocatedTo")

Gui, Add, ListView, Checked HWNDCategoryLV w%ListViewWidth% x20 y28 R10  -Hdr vComboBox2, Font  ;h190
AddRowLV2=0
FillCategory(TDL)
ComboX_Set( CategoryLV, "4LD esc space enter" CategoryBtn+0, "OnComboCategory")

Gui Show, ,Create New Task
GuiControl, Focus, Title
Return
#ifwinactive


GuiAddDropDown(Description, VariableName, Choice,XPos,Width,Categories)
{
	Global
	if (CurCol>Settings.Columns)
	{
		CurCol:=1
		GetX:=ColDX%CurCol%
		Gui, Add, Text, x%GetX% y+10, %Description%
	}
	else
	{
		GetX:=ColDX%CurCol%
		Gui, Add, Text, x%GetX% yp+%YPOffSet%, %Description%
	}
	GetX:=ColVX%CurCol%	
	
	
	Gui, Add, DropDownList, x%GetX% yp-%YPOffSet% v%VariableName% w%Width% %Choice%, %Categories%
	CurCol++
}

GuiAddEditTime(Description, VariableName, Default,XPos,Width,CurTimeUnits="H",TimeUnitSelectorLabel="")
{
	Global
		if (CurCol>Settings.Columns)
	{
...
;	Gui, Add, Text, xm y+10, %Description%
	NewWidth:=Width-24
	Gui, Add, Edit, x%GetX% yp-%YPOffSet% v%VariableName% w%NewWidth% r1 Number, %Default%
	Gui, Add, Button, x+0 g%TimeUnitSelectorLabel% v%VariableName%Button w24 h22 0x8000, %CurTimeUnits%
	
	CurCol++	
}

GuiAddDate(Description, VariableName, Default,XPos,Width,DateType=1, TimeVariable="")
{
	Global
	if (CurCol>Settings.Columns)
	{
	...
	if DateType=1
	{
		...
		Gui, Add, DateTime, x%GetX% yp-%YPOffSet% ChooseNone v%VariableName% w%Width% 2, %DateFormat%
	}
	else
	if DateType=2
	{
	...
	if DateType=3
	{
		...
		if (CurCol>Settings.Columns)
		{
			CurCol:=1
			GetX:=ColDX%CurCol%
			Gui, Add, Text, x%GetX% y+10, Time:
		}
		else
		{	
			GetX:=ColDX%CurCol%
			Gui, Add, Text, x%GetX% yp+%YPOffSet%, Time:
		}
			GetX:=ColVX%CurCol%
			Gui, Add, ComboBox, x%GetX% yp-%YPOffSet% v%TimeVariable% w%Width% , |00:00|01:00|02:00|03:00|04:00|05:00|06:00|07:00|08:00|09:00|10:00|11:00|12:00|13:00|14:00|15:00|16:00|17:00|18:00|19:00|20:00|21:00|22:00|23:00
			
	}
	CurCol++
}

GuiAddEdit(Description, VariableName, Default,XPos,Width,Number=0)
{
	Global
	if (CurCol>Settings.Columns)
	{
	...
	if Number
		Gui, Add, Edit, x%GetX% yp-%YPOffSet% v%VariableName% w%Width% r1 Number, %Default%
	else
		Gui, Add, Edit, x%GetX% yp-%YPOffSet% v%VariableName% w%Width% r1, %Default%

	CurCol++
}

TimeEstUnitSelector:
	Menu, TimeEstMenu, Add, m(inutes), MenuHandler  ;Lazy way to allow deletion of menu
	Menu, TimeEstMenu, Delete						;Lazy way to clear checkmarks
	Menu, TimeEstMenu, Add, m(inutes), MenuHandler
	if TimeEstUnits=m
			Menu, TimeEstMenu,Check, m(inutes)
	Menu, TimeEstMenu, Add, H(ours), MenuHandler
	if TimeEstUnits=H
			Menu, TimeEstMenu,Check, H(ours)
	Menu, TimeEstMenu, Add, D(ays), MenuHandler
	if TimeEstUnits=D
			Menu, TimeEstMenu,Check, D(ays)
	Menu, TimeEstMenu, Add, W(eeks), MenuHandler
	if TimeEstUnits=W
			Menu, TimeEstMenu,Check, W(eeks)
	Menu, TimeEstMenu, Add, M(onths), MenuHandler
	if TimeEstUnits=M
			Menu, TimeEstMenu,Check, M(onths)
	Menu, TimeEstMenu, Add, Y(ears), MenuHandler
	if TimeEstUnits=Y
			Menu, TimeEstMenu,Check, Y(ears)
	Menu, TimeEstMenu, Show  
return

TimeSpentUnitSelector:
	Menu, TimeSpentMenu, Add, m(inutes), MenuHandler  ;Lazy way to allow deletion of menu
	Menu, TimeSpentMenu, Delete						;Lazy way to clear checkmarks
	Menu, TimeSpentMenu, Add, m(inutes), MenuHandler
	if TimeSpentUnits=m
			Menu, TimeSpentMenu,Check, m(inutes)
	Menu, TimeSpentMenu, Add, H(ours), MenuHandler
	if TimeSpentUnits=H
			Menu, TimeSpentMenu,Check, H(ours)
	Menu, TimeSpentMenu, Add, D(ays), MenuHandler
	if TimeSpentUnits=D
			Menu, TimeSpentMenu,Check, D(ays)
	Menu, TimeSpentMenu, Add, W(eeks), MenuHandler
	if TimeSpentUnits=W
			Menu, TimeSpentMenu,Check, W(eeks)
	Menu, TimeSpentMenu, Add, M(onths), MenuHandler
	if TimeSpentUnits=M
			Menu, TimeSpentMenu,Check, M(onths)
	Menu, TimeSpentMenu, Add, Y(ears), MenuHandler
	if TimeSpentUnits=Y
			Menu, TimeSpentMenu,Check, Y(ears)
	Menu, TimeSpentMenu, Show  
return


MenuHandler:
	if (A_ThisMenu="TimeEstMenu")
	{
		TimeEstUnits:=SubStr(A_ThisMenuItem, 1 ,1)
		GuiControl,, TimeEstimateButton,  %TimeEstUnits%
		
	}
	if (A_ThisMenu="TimeSpentMenu")
	{
		TimeSpentUnits:=SubStr(A_ThisMenuItem, 1 ,1)
		GuiControl,, TimeSpentButton,  %TimeSpentUnits%
...
FileLinkSelect:
	...
	FileSelectFile, FileLink , 1, %Directory%, Select file link
	if errorlevel=0
	GuiControl,, Filelink,%Filelink%	
Return
...
Save:
	...
	FileRead, xml, %Filename%
	TDL:=loadXML(XML)
	nodes:=TDL.childnodes.Item(1).childnodes
	for node in nodes
	if node.nodename="TASK"
		for attrib In (node.attributes)
			if (attrib.name="ID") and (attrib.nodevalue=TaskID)
					MyNode:=Node
...
;Save categorical attribute	with FIXED Categories
	...
	StringCaseSense, On
	if (Settings.TimeSpent)
	{MyNode.setattribute("TIMESPENT",TimeSpent)
	If TimeSpentUnits=m
		TimeSpentUnits=I
	MyNode.setattribute("TIMESPENTUNITS",TimeSpentUnits)
	}
	if (Settings.TimeEst)	
	{
		MyNode.setattribute("TIMEESTIMATE",TimeEstimate)
		If TimeEstUnits=m
			TimeEstUnits=I
		MyNode.setattribute("TIMEESTUNITS",TimeEstUnits)
	}
	StringCaseSense, Off
...
;If Category<>
;{
	Gui, ListView, ComboBox2
	NumCategories=0
	NextCheck:=LV_GetNext(0, "Checked")
	while NextCheck>0
	{
		LV_GetText(TEST, NextCheck, 1)
		if NumCategories=0
			MyNode.setattribute("CATEGORY",Test)
		else
			MyNode.setattribute("CATEGORY" .  numcategories,Test)
		NumCategories++
		NextCheck:=LV_GetNext(NextCheck, "Checked")
		CategoryIndex:=TDLGetCategoryIndex(Categories, Test)
		If CategoryIndex<0
		{
			MaxIndex:=TDLGetCategoryMaxIndex(Categories, TEST)
			TDLAddCategory(TDL, "Category", "Category" . MaxIndex, TEST)	
		}
	}
	MyNode.setattribute("NUMCATEGORY",NumCategories)
;}

	Gui, ListView, ComboBox1
	NumCategories=0
	NextCheck:=LV_GetNext(0, "Checked")
	while NextCheck>0
	{
		LV_GetText(ThisAllocatedTo, NextCheck, 1)
		traytip, 1, %ThisAllocatedTo%
		sleep,1000
		if NumCategories=0
			MyNode.setattribute("PERSON",ThisAllocatedTo)
		else
			MyNode.setattribute("PERSON" .  numcategories,ThisAllocatedTo)
		NumCategories++
		NextCheck:=LV_GetNext(NextCheck, "Checked")
		CategoryIndex:=TDLGetCategoryIndex(Categories, ThisAllocatedTo)
		If CategoryIndex<0
		{
			MaxIndex:=TDLGetCategoryMaxIndex(Categories, ThisAllocatedTo)
			TDLAddCategory(TDL, "PERSON", "PERSON" . MaxIndex, ThisAllocatedTo)	
		}
	}
	MyNode.setattribute("NUMCATEGORY",NumCategories)
	;Save a Date
	if (Settings.DueDate) and (DueDate<>"")
	{
		IF (Settings.DueDate=3)
			DueDate:=FixDate3(DueDate,DueTime)
		MyNode.setattribute("DUEDATE",TDLDate(Duedate))
		MyNode.setattribute("DUEDATESTRING",TDLDateString(Duedate))
	}
	if (Settings.StartDate) and (StartDate<>"")
	{
		IF (Settings.StartDate=3)
			StartDate:=FixDate3(StartDate,StartTime)
		MyNode.setattribute("STARTDATE",TDLDate(Startdate))
		MyNode.setattribute("STARTDATESTRING",TDLDateString(Startdate))
	}
	if (Settings.CreationDate) and (CreationDate<>"")
	{
		IF (Settings.CreationDate=3)
			CreationDate:=FixDate3(CreationDate,CreationTime)
		MyNode.setattribute("CREATIONDATE",TDLDate(Creationdate))
		MyNode.setattribute("CREATIONDATESTRING",TDLDateString(Creationdate))
	}
	if (Settings.LastModDate) and (LastModDate<>"")
	{
		IF (Settings.LastModDate=3)
			LastModDate:=FixDate3(LastModDate,LastModTime)
		MyNode.setattribute("LASTMOD",TDLDate(LastModDate))
		MyNode.setattribute("LASTMODSTRING",TDLDateString(LastModDate))
	}
;Creation and modification dates are not user editable (well it can be  - but I cannot see why you would want to)	



	;Not required anymore as TDL handles the new task creation
	...
	TDL.save(FileName)
	run %PathToTDL% %FileName% -tid %TaskID%
	WinWaitActive, Timestamp Change,,5
	if errorlevel=0
		Send, Y	
Gui, Destroy
return
...
TDLCategories(BYREF TDL, Category)
{
	Categories= 
	for node in TDL.childNodes.Item(1).childNodes
	{
...
OnComboAllocatedTo(Hwnd, Event) {
	global 
	if (Event = "Hide") or (event="select")
	{
		NextCheck:=LV_GetNext(0, "Checked")
		Filler=
		while NextCheck>0
...
Category:
	...
	if (AddRowLV2=0) 
	{
		if  (Category <> "") and (Category<>Filler2)
		{
		LV_Add("Check", Category )
		AddRowLV2:=LV_GetCount()
		}
	}
	else
	{
		if (Category<> Filler2)
			{
				LV_Modify(AddRowLV2, "", category)
				traytip %Category%, %AddRowLV2% - %Filler2%
			}
...
AllocatedTo:
	...
	if (AddRowLV1=0) 
	{
		if  (AllocatedTo <> "") and (AllocatedTo<>Filler)
		{
		LV_Add("Check", AllocatedTo )
		AddRowLV1:=LV_GetCount()
		}
	}
	else
	{
		if (AllocatedTo<> Filler)
			LV_Modify(AddRowLV1, "", AllocatedTo)
	}
return

CategoryLVLabel:
	Gui, ListView, ComboBox2
	ComboX_Show(CategoryLV)
return
AllocatedToLVLabel:
	Gui, ListView, ComboBox1
	ComboX_Show(AllocatedToLV)
return


;Source: http://www.autohotkey.com/forum/viewtopic.php?t=22390
/*	Title:	ComboX
...
;http://www.autohotkey.com/forum/viewtopic.php?t=19489
;toralf
