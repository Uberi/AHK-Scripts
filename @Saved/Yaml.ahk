Yaml(YamlText,IsFile=1,YamlObj=0){ ; Version 1.0.0.8 http://www.autohotkey.com/forum/viewtopic.php?t=70559
  static
  static base:=Object("Dump","Yaml_Dump","Save","Yaml_Save","Add","Yaml_Add","Merge","Yaml_Merge","__Delete","__Delete","_Insert","_Insert","_Remove","_Remove","_GetCapacity","_GetCapacity","_SetCapacity","_SetCapacity","_GetAddress","_GetAddress","_MaxIndex","_MaxIndex","_MinIndex","_MinIndex","_NewEnum","_NewEnum","_Next","_Next","_HasKey","_HasKey","_Clone","_Clone","Insert","Insert","Remove","Remove","GetCapacity","GetCapacity","SetCapacity","SetCapacity","GetAddress","GetAddress","MaxIndex","MaxIndex","MinIndex","MinIndex","NewEnum","NewEnum","Next","Next","HasKey","HasKey","Clone","Clone","base",Object("__Call","Yaml_Call"))
  static BackupVars:="LVL,SEQ,KEY,SCA,TYP,VAL,CMT,LFL,CNT",IncompleteSeqMap
  local maxLVL:=0,LastContObj:=0,LastContKEY:=0,LinesAdded:=0
  ...
  Loop,Parse,YamlText,`n,`r
  {
    ...
    If InStr(A_LoopField,"#"){
      If (RegexMatch(A_LoopField,"^\s*#.*") || InStr(A_LoopField,"%YAML")=1) ;Comments only, do not parse
        continue
      else if RegExMatch(A_LoopField,"(---)?\s*?(-\s)?("".+""\s*:\s|'.+'\s*:\s|[^:""'\{\[]+\s*:\s)\s*([\|\>][+-]?)?\s*(!!\w+\s)?\s*("".+|'.+)$")&&!RegExMatch(A_LoopField,"[^\\]""\s+#")
    ...
    If _CNT {
      If Yaml_IsSeqMap(IncompleteSeqMap LoopField)
        LoopField:=IncompleteSeqMap LoopField,_CNT:=0,IncompleteSeqMap:=""
      else IncompleteSeqMap.=LoopField
    }
    If (LoopField="---"){
      Loop % (maxLVL)
        LVL%A_Index%:=""
    ...
    If (_LVL-__LVL>1) ;&&!(__SEQ&&__KEY!=""&&_KEY!=""))
       LVL%_LVL%:=LVL%_NXT% ,_LVL:=__LVL%_PRV%+1
    If (maxLVL<_LVL)
    ...
    If (!_CNT && (_CNT:=Yaml_Incomplete(Trim(_LFL))||Yaml_Incomplete(Trim(_VAL)))){
...
Yaml_Seq(obj,key,value,isVal=0){
  ...
  Loop,Parse,value
  {
    ...
    If (A_Index=Count)
      LF .= A_LoopField
    else if (!Quote&&!Bracket&&A_LoopField=","&&SubStr(value,A_Index+1,1)=A_Space){
      ContinueNext:=1
      LF:=LF
    } else {
      LF .= A_LoopField
