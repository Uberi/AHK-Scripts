...
Version 2.2 (Sat September 28, 2013)
Created: Sat March 02, 2013
Author: tidbit
Credit:
   AfterLemon  --- st_insert() %=& st_overwrite() bug fix. st_strip()
   ...
   Verdlin     --- A couple nifty forum-only functions.
...
| st_strip(string, searchFor [, direction, instance, searchFor2])|
...
/*
st_strip (Made by, AfterLemon. Thanks!)
   string     = String to get from
...
Example: st_strip("111|222|333.444|555|444.777|888|999", "|","L",4)
Outputs:  111|222|333.444|555
*/
st_strip(string, searchFor, direction="R", instance=1, searchFor2="")
{
	(direction = "b")?((searchFor2="")?SearchFor2:=searchFor:):
	n:=InStr(string,searchFor)
	StringReplace,t,string,%searchFor%,,UseErrorLevel
	(Errorlevel<instance)?R:=string:
	while --instance
		n := InStr(string,searchFor,,n+1)
	R:=R!=""?R:SubStr(string, 1, --n)
	(direction="R")?R:=SubStr(string,StrLen(R)+StrLen(searchFor)+1)
	R:=(direction = "b")?(((InStr(string,searchFor2,,n)!=0)?SubStr(string,n+2,InStr(string,searchFor2,,n)-n-2):SubStr(string,StrLen(R)+StrLen(searchFor)+1))):R
return R
}
...
st_jumble(Text, Weight=1, Delim = "`n", Omit = "")
{
