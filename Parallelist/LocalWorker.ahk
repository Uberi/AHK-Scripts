...
Clipboard .= A_ScriptHwnd . "``tReceived " . ParallelistTask.Data . "``n" ;wip: debug
        ...
        Sleep, 0
        Clipboard .= A_ScriptHwnd . "``tDid " . ParallelistTask.Data . "``n" ;wip: debug
        ...
        ParallelistProcessTask(Worker,Task,pJob,pWorker,Master)
        {
            ...
            ;set up the COPYDATASTRUCT structure
            VarSetCapacity(CopyDataStruct,4 + (A_PtrSize * 2)) ;structure contains an integer field and two pointer sized fields
            NumPut(Status,CopyDataStruct) ;insert the task status
            NumPut(Length,CopyDataStruct,A_PtrSize,"UInt") ;insert the length of the data to be sent
            NumPut(&ResultStruct,CopyDataStruct,A_PtrSize * 2) ;insert the address of the result structure
